            ...
            <form action="{{ route('tasks.store') }}" method="POST" class="space-y-6 p-6">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input type="date" name="deadline" id="deadline" value="{{ old('deadline', optional($task->deadline)->format('Y-m-d')) }}" class="mt-1 w-full rounded border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" required>
                    </div>
